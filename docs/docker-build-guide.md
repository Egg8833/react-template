...
## 部署專用分支 (deploy-ToWindow)

本專案使用固定的 `deploy-ToWindow` 分支處理所有生產環境的部署工作。此分支只包含部署所需的必要檔案：
...
此腳本會自動產生一個名為 `deploy-ToWindow` 的資料夾，其中只包含部署所需的檔案，保持部署環境的精簡和專注。

在 Linux 生產伺服器上，只需將產生的 `deploy-ToWindow` 資料夾複製到伺服器上，然後執行部署命令即可。
